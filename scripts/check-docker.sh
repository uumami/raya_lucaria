...
Run the Docker Compose verification path:
  ...
  RAYA_DOCKER_USER=<caller uid:gid> docker compose run --rm --env RAYA_INSTALL_PLAYWRIGHT_CHROMIUM=1 dev ./scripts/check-python.sh
...
run_docker compose run --rm --env RAYA_INSTALL_PLAYWRIGHT_CHROMIUM=1 dev ./scripts/check-python.sh
