...
Local execution SHALL respect execution policies and cache metadata when deciding whether to run, reuse, or refuse a target.
...
#### Scenario: Frozen policy refuses baseline execution
- **WHEN** a user explicitly runs a `frozen` policy target
- **THEN** local execution MUST fail until a future accepted frozen-output trust contract exists
