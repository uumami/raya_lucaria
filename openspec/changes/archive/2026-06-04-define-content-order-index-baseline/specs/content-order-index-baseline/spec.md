...
Rendered course content SHALL use visible ordered source entries under the configured `content/` directory.
...
The ordered content contract SHALL define which source files render and which files remain private, partial, or draft material.

#### Scenario: Ordered Markdown pages render
- **WHEN** a Markdown file under `content/` uses a valid numeric or appendix prefix and is not under a private or draft path
- **THEN** it MUST be included in the resolved navigation and rendered static site

#### Scenario: Private paths do not render
- **WHEN** Markdown files live under `_partials/`, `_drafts/`, `drafts/`, or use a leading underscore path segment
...
- **WHEN** a Markdown file under an ordered content directory lacks a valid rendered prefix and is not private or draft material
- **THEN** validation MUST fail with a diagnostic that recommends adding an order prefix or moving the file to a private or draft path
