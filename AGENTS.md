...
Use Python 3.10 for the current baseline. `packages/schema` owns schemas and validators; `packages/cli` owns the `raya` command; `packages/static` owns the Glintstone static builder. Current local course Search and static Graph discovery belong to the Glintstone static renderer under `docs/foundation/20_learning_renderer_contract.md`. TypeScript/web UI, backend, identity, dynamic study state, graph scopes, cross-course graph, graph editing, inferred relationships, expanded wikilink behavior, heading-anchor validation beyond the current renderer contract, and expanded external link policy remain out of scope until later proposals.
