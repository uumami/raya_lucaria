...
| Graph node dragging | Cytoscape node dragging and help text | Current graph implementation includes temporary SVG node dragging, but current foundation authority names pointer drag panning and graph readability cues rather than accepted node-position dragging | Candidate requiring contract decision | Do not present as accepted UX until a future foundation/role-doc loop either accepts, constrains, or removes it |
