...
| Calendar | `current` | Render a generated static agenda and month surface from manifest-declared `data/calendar.json` inside the persistent Course map and its local shell resources, with local view, month, event-kind, and task-type controls, owning-page anchors, graph-focus links, optional URL-only page focus, and Clear/Escape reset. Its visible name is `Calendar`; `/_raya/schedule/` is a compatibility URL only. |
