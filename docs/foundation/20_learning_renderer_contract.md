...
| Local course search | `current` | Render a static search workspace from generated page metadata only, using local JavaScript, approximate matching, optional URL-only exact page focus such as `?page=<page-id>`, keyboard, hover, and focus result inspection, control/results/context panels, deployment-neutral page links, and graph-focus links generated from stable page IDs. |
