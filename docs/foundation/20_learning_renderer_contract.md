...
The current shell uses an expanded course map, rendered as an expanded hierarchical course map by default on desktop browser load, keeps the article primary as the continuous reading surface, and supports medium-width and mobile article-first layout. Medium-width structural shells may start with the course map and right learning rail collapsed into explicit edge openers so fixed reader controls do not cover the article. reader pages have no reader top bar. reader commands live in the left course rail. discovery workspaces may keep command bars. Article pages may show reader breadcrumbs with the course home, ancestor pages, and current page generated from navigation data. Breadcrumb links are deployment-neutral static links and must not expose authored source paths.

The left course rail contains the course map, one compact Course Tools surface for static Search, Graph, Practice, Tasks, Schedule, right learning-rail context, local text-size, and local OpenDyslexic controls. The rail renders no separate Course Workspaces section, no visible Current, All, Scan, or Less map action buttons, and no reader focus controls. The course map is generated from current navigation data, can be filtered locally by rendered page labels, may show generated structural sequence numbers, may auto-orient the current page into the visible map region after load, and exposes per-branch disclosure controls for hierarchy scanning. Branch collapse is reader-local orientation state: course-scoped sessionStorage may restore collapsed course-map branches across refresh and same-tab page navigation, but this state must not become durable progress, mastery, recommendations, analytics, source data, generated artifact data, localStorage, cookies, network state, or cross-tab memory. Workspace handoffs in Course Tools may show static structural labels and may include page-focused Search, Graph, Practice, Tasks, or Schedule links when current artifact data supports those links. It does not collapse on hover; desktop and medium-width structural readers can collapse it through an explicit click control, and keyboard users can close it with Escape. Collapsed structural mode removes or avoids reserving the full course-map column so the article reclaims reading space, and exposes a minimal floating Map edge opener to restore the map. In this state, collapsed course-map content is removed from keyboard and assistive navigation until restored.

The shell may use coordinated, reduced-motion-aware visual transitions for explicit map and context state changes so the reader perceives one continuous workspace; these transitions are display state only and must not persist shell state, infer progress, or hide accessible content outside the documented collapsed desktop states. On phone-sized layouts, the course map may open as a non-persistent modal drawer with visible chrome, a close button, backdrop and Escape close paths, focus containment, background inertness, and temporary background scroll lock. The closed drawer is non-persistent, inert, hidden from assistive navigation, and must not hide the article or right learning rail from normal reading. Course-map shell collapse, orientation, workspace links, drawer state, drawer scroll lock, right-rail context state, and filter text are non-persistent UI state. Course-map branch expansion is the only accepted same-tab exception: it may store collapsed branch identifiers in course-scoped sessionStorage and must clear with the browser tab session. Reader pages may persist only accepted comfort keys such as `raya:open-dyslexic` and `raya:text-size`. Discovery workspaces may use discovery command bars, and discovery workspace comfort controls are volatile and must not write `raya:open-dyslexic`, `raya:text-size`, or shell state. The right learning rail collapse is available only when explicit collapse controls are visible; phone layouts keep the rail body visually and accessibly available when collapse controls are hidden, and Escape must not create an inert hidden rail state there. The shell may show structural page position such as `Page N of M`; this is course structure, not personal progress. The article may also end with larger Previous/Next sequence cards generated from the same ordered navigation data so students have a clear static reading path after finishing the page.

## Static Renderer Status

| Capability | Status | Static renderer behavior |
| --- | --- | --- |
| Course map and reading context | `current` | Render a hierarchical map from current navigation data in the left course rail, expanded by default on desktop when no same-tab branch state exists, locally filterable by page labels, able to show structural sequence numbers, able to auto-orient the current page into view, able to expose per-branch disclosure controls, able to restore collapsed branch identifiers from course-scoped sessionStorage across refresh and same-tab page navigation, and desktop/medium-collapsible through an explicit click control or Escape into a minimal floating Map edge opener that restores the full map without reserving a shell grid column. |
...
Generated Search, Graph, Practice, Tasks, and Schedule pages may share discovery workspace chrome with discovery command bars that show the current course title, identify the workspace, link back to the course, link between discovery workspaces, and expose local volatile text-size and `OpenDyslexic` controls. Search, Practice, Tasks, and Schedule may also use static workspace regions for controls, results, and public context summaries so desktop readers can scan without leaving the page. Search, Practice, Tasks, and Schedule text filters may use local approximate matching over public payload fields and visible text to help find likely public results despite small spelling mistakes. Search result context, Practice object context, Tasks object context, and Schedule item context may follow transient active cards selected by keyboard movement, pointer hover, or focus on existing links. Those context panels may expose static action links generated from the same public payload, such as the owning page, graph focus, and sibling workspace handoffs when the active public item supports them. When Search, Practice, Tasks, or Schedule opens from a valid `?page=<page-id>` handoff, the control region may show a compact visible notice naming the focused public page, the current visible count, and the Clear reset path. That notice is structural URL context only and must hide when no valid page focus exists or after Clear/Escape restores the full static workspace. This chrome, active context, context actions, page-focus notice, and approximate matching are static page structure. Discovery workspace comfort controls are volatile and must not write `raya:open-dyslexic`, `raya:text-size`, shell state, search, graph, practice, task, or schedule state. Discovery workspaces must not load the course shell script, show a course map control without a course map, fetch external resources, or turn structural workspace labels into progress, ranking, or recommendation language.
...
Reader controls may use local `OpenDyslexic` resources, a local text-size comfort preference, keyboard-reachable controls in the left course rail, a static Search handoff, volatile desktop right-rail context visibility, copyable fenced code blocks, and previous/next page keyboard navigation from generated sequence links. They must work from static files and must not depend on fetch/XHR requests, accounts, a backend, CDN resources, external font requests, personal progress, adaptive recommendations, or browser-side MathJax conversion. Local storage may be used only for reader comfort preferences such as `raya:open-dyslexic` and `raya:text-size`; sessionStorage may be used only for course-scoped collapsed course-map branch identifiers. Browser storage must not store course progress, answers, mastery, recommendations, graph state, right-rail context state, shell collapse state, skin authority, or authored content.
