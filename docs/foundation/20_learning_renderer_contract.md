...
| --- | --- | --- |
| Course map and reading context | `current` | Render a hierarchical map from current navigation data in the left course rail, expanded by default when the approved `894px+` reader geometry is active and no same-tab branch state exists, locally filterable by page labels, able to show structural sequence numbers, able to auto-orient the current page into view, able to expose per-branch disclosure controls, able to restore collapsed branch identifiers from course-scoped sessionStorage across refresh and same-tab page navigation, and able to collapse through explicit controls into a minimal floating Map edge opener that restores the full map without reserving a shell grid column while narrower article-first layouts stay primary below that breakpoint. |
...
| Reader controls | `current` | Use local OpenDyslexic resources, keyboard-reachable controls in the left course rail, a static Search handoff, volatile desktop and medium-width right-rail Context behavior, copyable fenced code blocks, and previous/next page keyboard navigation from generated sequence links. |
