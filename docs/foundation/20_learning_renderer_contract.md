...
| Discovery workspace chrome | `current` | Render shared static course chrome on generated Search, Graph, and Practice surfaces with a course link, cross-workspace links, local comfort controls, compact mobile layout, and no shell script dependency. |
...
| Official Practice workspace | `current` | Render a generated static discovery surface at `_raya/practice/index.html` over already accepted official objects from `data/official.json` and page-owned `_official/` source objects, with local filters, local links back to owning page anchors, and graph focus links. |
