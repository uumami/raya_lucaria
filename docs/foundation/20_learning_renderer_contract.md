...
| --- | --- | --- |
| Course map and reading context | `current` | Render a hierarchical map from current navigation data in the left course rail, expanded by default when the approved `894px+` reader geometry is active and no same-tab shell state exists, locally filterable by page labels, able to show structural sequence numbers, able to auto-orient the current page into view, able to expose per-branch disclosure controls, able to restore collapsed branch identifiers and explicit course-map expanded/collapsed state from course-scoped sessionStorage across refresh and same-tab page navigation, and able to collapse through explicit controls into a minimal floating Map edge opener that restores the full map without reserving a shell grid column while narrower article-first layouts stay primary below that breakpoint. |
