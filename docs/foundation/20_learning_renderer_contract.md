...
| Discovery workspace chrome | `current` | Render the persistent Course map and its local shell resources on generated Search, Graph, Practice, Tasks, and Schedule surfaces. Generate course-home, tree, and workspace links relative to the workspace output path; mark only the active workspace tile current; omit reader-only Context; preserve volatile workspace controls, filters, results, context regions, and focused-page strips; and do not fetch external resources or write learner, source, artifact, or preference state. |
