...
| Calendar | `current` | Render a generated static agenda and month surface from manifest-declared `data/calendar.json`, with local filters by event kind and task type, keyboard, hover, and focus inspection, owning-page anchors, graph-focus links, and optional URL-only page focus. Its visible name is `Calendar`; `/_raya/schedule/` is a compatibility URL only. |
...
only. Calendar may filter locally by event kind and task type, show transient
context on keyboard movement, hover, or focus, link to owning page anchors and
graph focus, and accept non-persistent `?page=<page-id>` focus. Agenda and
month controls, filters, focus, and Clear/Escape reset are temporary display
state. Calendar is not source authority, a machine calendar feed, a reminder,
...
runtime, make no network requests, load external resources, persist state, or
expose private paths or hidden answer/support content.
